package mspebble // import "mossserver/internal/com/macrosan/database/pebble"
...
func newBatchPebble(isMetaPool bool, lun string, db *pebble.DB) *BatchPebble {
	bp := &BatchPebble{
		...
		dataProc:   make(chan *BatchRequest, 4096),
...
func (bp *BatchPebble) flush(requests []*BatchRequest) {
	// WriteBatch wraps an IndexedBatch: Get() reflects in-batch writes.
	batch := NewWriteBatch(bp.db, bp.lun)
	defer batch.Close() // always release, even on Commit failure
