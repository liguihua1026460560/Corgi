...
func (a *AIO) wait(to timespec, completed []RequestId) (int, error) {
	...
	for {
		...
		if x == uintptr(0) {
			// timeout 才可能发生；zeroTime 时 toPtr=nil，理论上不会正常返回 0
			return 0, nil
		}

		if x > uintptr(activeLen) {
